console.log("NSS v1 Wizard (Core Buyer – Step 1) loaded ✅");
...
const QUESTIONS = [
  ...
  { key:"role", section:"Profile", prompt:"Where do you sit in the chain — end buyer, distributor, trader, or agent?", type:"single", options:["End Buyer","Distributor","Trader","Agent"], allowText:true, placeholder:"If different, type here" },
...
function renderDashboard(){
  ...
  app.innerHTML = `
    <div style="padding:20px;">
      ...
      <div style="opacity:0.9;">Core Buyer Wizard (Step 1)</div>
    ...
    <div style="display:flex;gap:14px;align-items:stretch;">
      <div style="flex:2;background:#132A3A;border:1px solid rgba(198,169,74,0.45);padding:16px;border-radius:10px;">
        ...
        ${q.options ? `<div style="margin-top:12px;display:flex;gap:10px;flex-wrap:wrap;">
          ${q.options.map(o=>`<button class="chip ${isSelected(a.value,o,q.type) ? "chip--on":""}" type="button" onclick="window.pick(${JSON.stringify(o)})">${escapeHtml(o)}</button>`).join("")}
        </div>` : ""}
      ...
      <div style="flex:1;background:#132A3A;border:1px solid rgba(198,169,74,0.45);padding:16px;border-radius:10px;">
        ...
        <div style="margin-top:12px;">
          <div style="font-size:12px;opacity:0.75;margin-bottom:6px;">Current Field</div>
          <div class="assistBox">
            <div class="hint"><b>${escapeHtml(labelForKey(q.key))}</b></div>
            <div class="hint">${a.pending ? "[PENDING]" : escapeHtml(String(a.value || ""))}</div>
          </div>
        </div>
...
function isSelected(value, opt, type){
  return String(value) === String(opt);
}

/* ========= actions ========= */
window.setTemp = (t) => { const prev = state.temperature; state.temperature = t; log(`Temp: ${prev} → ${t}`); renderCallMode(); };
...
window.setPending = (checked) => {
  ...
  a.pending = checked;
  state.answers[q.key] = a;
  log(`${labelForKey(q.key)} marked ${checked ? "PENDING" : "ANSWERED"}`);
  renderCallMode();
};
